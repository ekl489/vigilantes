<template>
    <div id="searchbar">
        ...
        <ul class="menu-list">
            ...
            <!-- Ground pages -->
            <li v-for="page_link in pages" v-show="page_link.SearchQuery_Show && page_link.LinkDisplayStyle > 0"><a href="#" v-on:click="setPage(page_link)">{{ page_link.title }}</a></li>

            <!-- sub pages -->
            <span v-for="page_link in pages">
                <li v-for="sub_page_link in page_link.pages" v-show="sub_page_link.SearchQuery_Show && sub_page_link.LinkDisplayStyle > 0">
            ...
            <span v-for="a in pages">
                <span v-for="b in a.pages" >
                    <li v-for="c in b.pages" v-show="c.SearchQuery_Show  && c.LinkDisplayStyle > 0">
            ...
            <span v-for="a in pages">
                <span v-for="b in a.pages" >
                    <span v-for="c in b.pages" >
                        <li v-for="d in c.pages" v-show="d.SearchQuery_Show && d.LinkDisplayStyle > 0">
            ...
            search: function() {
                ...
                for(var x = 0; x < this.pages.length; x++){
                    if(stringSimilarity.compareTwoStrings(this.query, this.pages[x].title) > 0){
                        this.pages[x].SearchQuery_Show = true
                    }
                    else{
                        this.pages[x].SearchQuery_Show = false
                    }
                    ...
                    if(this.pages[x].hasOwnProperty('pages')){
                        ...
                        for(var y = 0; y < this.pages[x].pages.length; y++){
                            if(stringSimilarity.compareTwoStrings(this.query, this.pages[x].pages[y].title) > 0){
                                this.pages[x].pages[y].SearchQuery_Show = true
                            }
                            else{
                                this.pages[x].pages[y].SearchQuery_Show = false
                            }
                            ...
                            if(this.pages[x].pages[y].hasOwnProperty('pages')){
                                ...
                                for(var z = 0; z < this.pages[x].pages[y].pages.length; z++){
                                    if(stringSimilarity.compareTwoStrings(this.query, this.pages[x].pages[y].pages[z].title) > 0){
                                        this.pages[x].pages[y].pages[z].SearchQuery_Show = true
                                    }
                                    else{
                                        this.pages[x].pages[y].pages[z].SearchQuery_Show = false
                                    }
                                    ...
                                    if(this.pages[x].pages[y].pages[z].hasOwnProperty('pages')){
                                        ...
                                        for(var w = 0; w < this.pages[x].pages[y].pages[z].pages.length; w++){
                                            if(stringSimilarity.compareTwoStrings(this.query, this.pages[x].pages[y].pages[z].pages[w].title) > 0){
                                                this.pages[x].pages[y].pages[z].pages[w].SearchQuery_Show = true
                                            }
                                            else{
                                                this.pages[x].pages[y].pages[z].pages[w].SearchQuery_Show = false
                                            }
                                        }
                                    }
                                }
